...
def main(test_folder, prod_folder, do_test, spec_file_suffix=None):
    ...
    if do_test:
        res = deployment.submit_jobs_for_all_pipelines(apiClient, test_folder, artifact_uri, libraries,
                                                       cloud, spec_file_suffix)
        if not res:
    ...
    if path.exists(prod_folder):
        deployment.create_or_update_production_jobs(apiClient, prod_folder, run_id, artifact_uri, libraries,
                                                    cloud, model_name, ["production"], model_version, spec_file_suffix)
